...
int ct_program_setup(ct_program_t *program)
{
	...
	if (vka_setup_vulkan(&(program->vulkan))) { return -1; }
	...
	// Scene uniform:
	if (vka_create_buffer(&(program->vulkan), &(program->scene_uniform_buffer))) { return -1; }
	...
	// Pipelines:
	if (vka_create_pipeline(&(program->vulkan), &(program->node_pipeline))) { return -1; }
	if (vka_create_pipeline(&(program->vulkan), &(program->arc_pipeline))) { return -1; }
	if (vka_create_pipeline(&(program->vulkan), &(program->mesh_pipeline))) { return -1; }
...
{
	program->scalar_function = ct_tree_scalar_function_y;
	...
	// Rendering info:
	program->render_info.colour_clear_value.color.float32[0] = 0.075f;
	program->render_info.colour_clear_value.color.float32[1] = 0.075f;
	program->render_info.colour_clear_value.color.float32[2] = 0.1f;
	...
	program->node_pipeline.num_vertex_bindings = 2;
	program->node_pipeline.strides[0] = 4 * sizeof(float);
	program->node_pipeline.strides[1] = sizeof(int8_t);
	program->node_pipeline.num_vertex_attributes = 2;
	program->node_pipeline.bindings[0] = 0;
	program->node_pipeline.bindings[1] = 1;
	program->node_pipeline.formats[0] = VK_FORMAT_R32G32B32A32_SFLOAT;
	program->node_pipeline.formats[1] = VK_FORMAT_R8_SINT;
	...
	program->arc_pipeline.strides[0] = 4 * sizeof(float);
	...
	program->mesh_pipeline.strides[0] = 4 * sizeof(float);
	...
	strcpy(program->node_buffer_index.name, "CT node index buffer");
	program->node_buffer_index.allocation = &(program->node_allocation);
	program->node_buffer_index.usage = VKA_BUFFER_USAGE_INDEX;
	program->node_buffer_index.index_type = VK_INDEX_TYPE_UINT32;
	...
	program->join_arc_buffer.usage = VKA_BUFFER_USAGE_INDEX;
	program->join_arc_buffer.index_type = VK_INDEX_TYPE_UINT32;
	...
	program->split_arc_buffer.usage = VKA_BUFFER_USAGE_INDEX;
	program->split_arc_buffer.index_type = VK_INDEX_TYPE_UINT32;
...
int ct_program_object_setup(ct_program_t *program)
{
	...
	fprintf(stdout, "\n");
	print_time_end(stdout, &time, "(mesh load):\t\t\t\t");

	get_time(&time);
	if (ct_mesh_calculate_edges(&(program->mesh), program->error)) { return -1; }
	print_time_end(stdout, &time, "(edge calculation):\t\t\t");

	get_time(&time);
	if (ct_mesh_check_manifold(&(program->mesh), program->error)) { return -1; }
	print_time_end(stdout, &time, "(manifold check):\t\t\t");
	...
	print_time_end(stdout, &time, "(scalars setup):\t\t\t");
	...
	if (ct_program_prepare_mesh(program, &gpu_mesh)) { return -1; }
	print_time_end(stdout, &time, "(GPU mesh setup):\t\t\t");
	...
	print_time_end(stdout, &time, "(join tree):\t\t\t\t");
	get_time(&time);
	...
	print_time_end(stdout, &time, "(split tree):\t\t\t");
	#ifdef CT_DEBUG
	fprintf(stdout, "\n*************\n");
	...
	print_time_end(stdout, &time, "(tree reduction):\t\t\t");

	#ifdef CT_DEBUG
	fprintf(stdout, "\n*********************\n");
	fprintf(stdout, "* Reduced join tree *\n");
	fprintf(stdout, "*********************\n");
	ct_tree_print_short(stdout, &(program->join_tree));

	fprintf(stdout, "\n**********************\n");
	fprintf(stdout, "* Reduced split tree *\n");
	fprintf(stdout, "**********************\n");
	ct_tree_print_short(stdout, &(program->split_tree));
	#endif
	...
	print_time_end(stdout, &time, "(contour tree):\t\t\t");
	...
	// Reconstruct join and split trees:
...
void ct_program_object_shutdown(ct_program_t *program)
{
	...
	vka_destroy_buffer(&(program->vulkan), &(program->node_buffer_index));
...
int ct_program_prepare_mesh(ct_program_t *program, ct_mesh_gpu_ready_t *gpu_mesh)
{
	...
	gpu_mesh->uvs = malloc(gpu_mesh->num_vertices * sizeof(ct_uv_t));
	gpu_mesh->faces = malloc(gpu_mesh->num_faces * sizeof(ct_face_gpu_ready_t));
	if (!gpu_mesh->vertices || !gpu_mesh->normals || !gpu_mesh->uvs || !gpu_mesh->faces)
	{
		snprintf(program->error, NM_MAX_ERROR_LENGTH,
	...
	// Copy vertices and faces directly:
	memcpy(gpu_mesh->vertices, program->mesh.vertices, gpu_mesh->num_vertices *
							sizeof(ct_vertex_t));
	...
	for (uint32_t i = 0; i < gpu_mesh->num_faces; i++)
	{
		for (int j = 0; j < 3; j++)
		{
			gpu_mesh->faces[i][j] = program->mesh.faces[i][j].v;
		}
	}
	...
	// Put scalar values into U coordinates:
	for (uint32_t i = 0; i < gpu_mesh->num_vertices; i++)
	{
		gpu_mesh->uvs[i].u = program->join_tree.nodes[
			program->join_tree.nodes[i].vertex_to_node].value;
	}
...
{
	// Per-node info buffers:
	program->node_buffer_positions_scalars.size = gpu_mesh->num_vertices * 4 * sizeof(float);
	...
	program->node_buffer_index.size = program->contour_tree.num_nodes * sizeof(uint32_t);
	if (vka_create_buffer(&(program->vulkan), &(program->node_buffer_index)))
	{
		return -1;
	}
	if (vka_get_buffer_requirements(&(program->vulkan), &(program->node_buffer_index)))
	{
		return -1;
	}
	...
	if (vka_create_buffer(&(program->vulkan), &(program->join_arc_buffer))) { return -1; }
	...
	if (vka_create_buffer(&(program->vulkan), &(program->split_arc_buffer))) { return -1; }
	...
	if (vka_create_buffer(&(program->vulkan), &(program->contour_arc_buffer))) { return -1; }
	...
	if (vka_create_buffer(&(program->vulkan), &(program->mesh_buffer_index))) { return -1; }
	...
	if (vka_create_buffer(&(program->vulkan), &(program->mesh_buffer_normals))) { return -1; }
...
{
	// Per-node info buffers:
	if (vka_create_allocation(&(program->vulkan), &(program->node_allocation))) { return -1; }
	if (vka_bind_buffer_memory(&(program->vulkan), &(program->node_buffer_positions_scalars)))
	{
		return -1;
	}
	if (vka_bind_buffer_memory(&(program->vulkan), &(program->node_buffer_index)))
	{
		return -1;
	}
	...
	// Per-arc info buffers:
	if (vka_create_allocation(&(program->vulkan), &(program->arc_allocation))) { return -1; }
	if (vka_bind_buffer_memory(&(program->vulkan), &(program->join_arc_buffer))) { return -1; }
	if (vka_bind_buffer_memory(&(program->vulkan), &(program->split_arc_buffer))) { return -1; }
	...
	// Object mesh buffers:
	if (vka_create_allocation(&(program->vulkan), &(program->mesh_allocation))) { return -1; }
...
int ct_program_upload_object_data(ct_program_t *program, ct_mesh_gpu_ready_t *gpu_mesh)
{
	...
	if ((vka_create_buffer(&(program->vulkan), &staging_buffer)) ||
		(vka_get_buffer_requirements(&(program->vulkan), &staging_buffer)) ||
		(vka_create_allocation(&(program->vulkan), &staging_allocation)) ||
		(vka_bind_buffer_memory(&(program->vulkan), &staging_buffer)) ||
		(vka_map_memory(&(program->vulkan), &staging_allocation)))
	{
	...
	float *positions_scalars = malloc(gpu_mesh->num_vertices * 4 * sizeof(float));
	...
	for (uint32_t i = 0; i < gpu_mesh->num_vertices; i++)
	{
		...
		positions_scalars[(i * 4) + 3] = gpu_mesh->uvs[i].u;
	}
	...
	// Upload node indices:
	uint32_t *node_indices = malloc(program->contour_tree.num_nodes * sizeof(uint32_t));
	if (!node_indices)
	{
		snprintf(program->error, NM_MAX_ERROR_LENGTH,
			"Could not allocate memory for upload of node indices.");
		vka_destroy_buffer(&(program->vulkan), &staging_buffer);
		vka_destroy_allocation(&(program->vulkan), &staging_allocation);
		return -1;
	}
	...
	{
		node_indices[i] = program->contour_tree.nodes[i].node_to_vertex;
	}
	if (ct_program_upload_helper(program, &staging_allocation, &staging_buffer,
			&(program->node_buffer_index), (uint8_t *)(node_indices)))
	{
		free(node_indices);
		vka_destroy_buffer(&(program->vulkan), &staging_buffer);
		vka_destroy_allocation(&(program->vulkan), &staging_allocation);
	...
	int8_t *types = malloc(program->contour_tree.num_nodes * sizeof(int8_t));
	if (!types)
	{
		snprintf(program->error, NM_MAX_ERROR_LENGTH,
	...
	for (uint32_t i = 0; i < program->join_tree.num_nodes; i++)
	{
		...
		{
			arc_endpoints[arc_index] = program->join_tree.nodes[i].node_to_vertex;
			arc_endpoints[arc_index + 1] = program->join_tree.nodes[
					program->join_tree.arcs[j]].node_to_vertex;
			arc_index += 2;
		}
	}
	if (ct_program_upload_helper(program, &staging_allocation, &staging_buffer,
	...
	for (uint32_t i = 0; i < program->split_tree.num_nodes; i++)
	{
		...
		{
			arc_endpoints[arc_index] = program->split_tree.nodes[i].node_to_vertex;
			arc_endpoints[arc_index + 1] = program->split_tree.nodes[
					program->split_tree.arcs[j]].node_to_vertex;
			arc_index += 2;
		}
	}
	if (ct_program_upload_helper(program, &staging_allocation, &staging_buffer,
	...
	for (uint32_t i = 0; i < program->contour_tree.num_nodes; i++)
	{
		...
		{
			arc_endpoints[arc_index] = program->contour_tree.nodes[i].node_to_vertex;
			arc_endpoints[arc_index + 1] = program->contour_tree.nodes[
					program->contour_tree.arcs[j]].node_to_vertex;
			arc_index += 2;
		}
	}
	if (ct_program_upload_helper(program, &staging_allocation, &staging_buffer,
...
int ct_program_render(ct_program_t *program)
{
	...
	{
		// Join tree:
		vertex_buffers[1].buffer = program->join_node_buffer_types.buffer;
	}
	else if (program->tree_display == 2)
	{
		// Split tree:
		vertex_buffers[1].buffer = program->split_node_buffer_types.buffer;
	}
	else
	{
		// Contour tree:
		vertex_buffers[1].buffer = program->contour_node_buffer_types.buffer;
	}
	vka_bind_pipeline(&(program->vulkan.command_buffers[program->vulkan.current_frame]),
								&(program->node_pipeline));
	vka_bind_vertex_buffers(&(program->vulkan.command_buffers[program->vulkan.current_frame]),
						&(program->node_buffer_index), 2, vertex_buffers);
	vka_draw_indexed(&(program->vulkan.command_buffers[program->vulkan.current_frame]),
